...
We evaluated Auto-ChemInstruct on a 20-hypothesis generation run with full pipeline (3 bootstrap iterations, cosine temperature schedule). The system produced 16 valid hypotheses, of which 14 passed all verification stages (87.5\% pass rate), yielding 14 DPO preference pairs with causal reflection traces. Dataset compilation produced 11 training, 1 validation, and 2 test pairs with stratified splitting by reaction type.
...
Structural diversity analysis of the 14-pair dataset revealed strong molecular diversity. Across 32 unique molecules extracted from passed reactions, we computed the following:
...
\textbf{Tanimoto Diversity}: Pairwise Tanimoto similarity using Morgan fingerprints (radius=2, 2048 bits) yielded a mean similarity of 0.112, corresponding to a structural diversity score of 0.888. The maximum pairwise similarity was 1.000 (identical products formed via different routes), while the minimum was 0.000 (completely orthogonal structures).

\textbf{Scaffold Diversity}: Murcko scaffold decomposition identified 18 unique scaffolds from 32 molecules (56\% scaffold diversity), indicating that the hypothesis agent generates structurally diverse cores rather than minor substituent variations.

\textbf{Reaction Type Distribution}: The current dataset is dominated by general organic transformations. Enhanced prompt engineering and multi-hop RAG enrichment are expected to increase reaction type diversity in scaled runs.
